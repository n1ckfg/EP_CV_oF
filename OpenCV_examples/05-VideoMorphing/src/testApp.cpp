...
void testApp::setup()
{
	...
	float decimate = 0.3;
	imageDecimated.allocate( color1.width * decimate, color1.height * decimate );
	imageDecimated.scaleIntoMe( color1, CV_INTER_AREA );
	gray1 = imageDecimated;

	color2.setFromPixels( imageOf2 );
	imageDecimated.scaleIntoMe( color2, CV_INTER_AREA );
	gray2 = imageDecimated;
	...
	//Uncomment to see opticval flow (will be shown in separate windows)
	//imshow( "x", flowPlanes[0] * (-0.2) );
	//imshow( "y", flowPlanes[1] * (-0.2) );
...
void testApp::updateMorph( float morphValue, int morphImageIndex )
{
	mapX.allocate( w, h );
	mapY.allocate( w, h );
	float *flowXPixels = flowX.getPixelsAsFloats();	//Get pointers to pixels data
	float *flowYPixels = flowY.getPixelsAsFloats();
